...
if [ "$HOOK_EVENT_NAME" = "UserPromptSubmit" ]; then
  ...
  if [ -f "$MARKER" ]; then
    ...
    if [ "$GROWTH" -ge 300 ]; then
      ...
      printf 'WARN:[session-reflection] Mid-session checkpoint: %d new transcript lines since last reflection. Before starting this task — any NEW lessons, repeated mistakes, or changed approach? If yes, write a brief note to %s/. If nothing new, continue.' "$GROWTH" "$REFLECTIONS_DIR"
    fi
  fi
  exit 0
fi
...
if [ "$SUBSTANTIVE" = false ]; then
  ...
  printf 'WARN:[session-reflection] Short session ending (%s turns, %s tool uses). Any lesson learned worth capturing? If yes, write to %s/. If not, just stop.' "$ASSISTANT_TURNS" "$TOOL_USES" "$REFLECTIONS_DIR"
  exit 0
fi
...
REASON="[session-reflection] Session: ${ASSISTANT_TURNS} turns, ${TOOL_USES} tool uses, ${TRANSCRIPT_LINES} transcript lines. Before stopping: write a reflection report to ${REFLECTIONS_DIR}/${DATESTAMP}.md using this template:
${FILLED}
Then stop."
