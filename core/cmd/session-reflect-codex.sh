...
if ! mkdir "$STATE_DIR/${_safe_session}.lock" 2>/dev/null; then
  exit 0
fi
printf 'claimed' > "$DEDUP_FILE" 2>/dev/null
rmdir "$STATE_DIR/${_safe_session}.lock" 2>/dev/null

command -v codex >/dev/null 2>&1 || { rm -f "$DEDUP_FILE" 2>/dev/null; exit 0; }
[ -f "$TRANSCRIPT_PATH" ] || { printf 'missing-transcript' > "$DEDUP_FILE" 2>/dev/null; exit 0; }
...
TARGET_FILE="$REFLECTIONS_DIR/${DATESTAMP}.md"
