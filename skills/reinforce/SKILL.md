---
name: reinforce
description: Process accumulated session reflections into actionable improvements via plan mode — analyze patterns, propose changes, user reviews, then execute. Works in Claude Code and Codex.
---
...
**Trigger:** User says `/reinforce` in Claude Code, `$reinforce` in Codex, asks to process reinforce reflections, or `reflection-reminder` guard signals accumulated reflections at session start.
...
4. Read current agent instruction files when present: `AGENTS.md` (Codex), `CLAUDE.md` (Claude Code), and nested overrides relevant to the current directory. These are needed to avoid duplicate improvements and to count existing rules.
...
Enter plan mode if the host supports it. In Codex, use the current plan/review workflow and do not apply changes until the user approves the plan. Write the plan with:
